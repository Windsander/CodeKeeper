...
| 管线编排       | 项目级可视化管线画布：Role/触发器/知识/外部 Agent 均为节点，编辑即写回                      |
|                | `.codekeeper/pipeline.yaml` 并热加载；角色节点可钻取为内部 stage 子图。                     |
| 外部 Agent     | A2A 词汇任务信封 + subprocess/A2A/MCP 传输适配器；MCP 门面（127.0.0.1+token）               |
|                | 向外部 Agent 暴露管线触发与知识召回。                                                       |
| 项目智库       | 策展知识双正本（项目内 knowledge/ 入库 + 本地私有）+ EverOS 投影检索；                      |
|                | 经验蒸馏经人审闸门入库。                                                                    |
...
桌面工作台以项目为主要容器：全局导航集中到仪表盘、智库、系统状态和设置；角色配置在项目管线画布的节点检查器中完成。项目详情包含管线、运行、知识、归档和设置五个视图，日志从设置页打开独立窗口。
...
4. 进入项目详情，在“管线”画布的角色节点检查器中填写 GitLab 项目地址、Token、筛选条件和角色参数。
