...
async function renderAndStabilize(ui: React.ReactElement) {
  const result = render(<LayoutProvider>{ui}</LayoutProvider>);
  await act(async () => {
    for (let i = 0; i < 5; i++) {
...
function createProject(overrides?: Partial<Project>): Project {
  return {
    id: 'p1',
    name: 'test-project',
    rootPath: join('virtual-workspace', 'test-project'),
...
function setupMocks() {
  mockInvoke.mockImplementation(
    async (method: string, params?: { role?: string; projectId?: string }) => {
      switch (method) {
        ...
        case 'project.soul.get':
          return { soul: { content: '', sourcePath: join('virtual-workspace', 'soul.md') } };
...
describe('RolePage', () => {
  ...
  it('archiver 页面显示 项目知识 标题', async () => {
    setupMocks();
    await renderAndStabilize(<RolePage role="archiver" />);
    await waitFor(() => {
