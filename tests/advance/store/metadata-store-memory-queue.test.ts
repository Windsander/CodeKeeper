...
describe('MetadataStore pending_memory_writes', () => {
  ...
  function sampleWrite(
    overrides: Partial<Parameters<MetadataStore['insertPendingMemoryWrite']>[0]> = {}
  ) {
    return {
      id: 'id-1',
      projectId: 'proj-1',
  ...
  it('相同 content_hash 重复入队会合并并累加 failure_count', () => {
    ...
    store.insertPendingMemoryWrite(
      sampleWrite({ failureCount: 2, nextRetryAt: 2000, lastError: 'error-2' })
    );
  ...
  it('listPendingMemoryWrites 只返回已到重试时间的任务', () => {
    ...
    store.insertPendingMemoryWrite(
      sampleWrite({ id: 'b', contentHash: 'hash-2', nextRetryAt: 3000 })
    );
