...
export class MemoryClient implements IMemoryClient {
  ...
  async recordFindingCases(cases: MemoryFindingCase[]): Promise<void> {
    if (cases.length === 0) return;
    await this.callTool('record_finding_cases', { context: this.context, cases });
  }

  async recallFindingCase(key: string): Promise<string[]> {
    return this.parseRecallResult(
      await this.callTool('recall_finding_case', { context: this.context, key })
    );
  }
