...
/** * 基于 LlmClient 的 Maintainer 判别辅助实现 */ export class LlmMaintainerLocalJudge implements MaintainerLocalJudge {
  ...
  async reassessSemanticIdentity(
    ...
    try {
      ...
      if (!body || typeof body.likelySame !== 'boolean') {
        return { kind: 'unreliable', reason: 'LLM 返回了不可解析的语义重识别结果' };
      }
    ...
    try {
      const json = await this.llmClient.completeJson(
        ...
        this.stuckSystem(),
        {
          type: 'object',
          properties: {
            suggestion: { type: 'string', enum: ['continue', 'refocus', 'broaden', 'stop'] },
      ...
      if (
        ...
        return { kind: 'unreliable', reason: 'LLM 返回了不可解析的卡点校正结果' };
      }
      return {
  ...
  async assistAlreadyFixedCheck(
    ...
    const payload: AlreadyFixedPromptPayload = { findingDescription, currentCodeContextHint };
    try {
      const json = await this.llmClient.completeJson(
      ...
      if (!body || typeof body.likelyAlreadyFixed !== 'boolean') {
        return { kind: 'unreliable', reason: 'LLM 返回了不可解析的 already-fixed 辅助结果' };
      }
      return {
  ...
  async preFilterScope(
    ...
    const payload: PreFilterScopePromptPayload = { findingDescription, findingFile, findingLine };
    try {
      const json = await this.llmClient.completeJson(
      ...
      if (!body || typeof body.scope !== 'string') {
        return { kind: 'unreliable', reason: 'LLM 返回了不可解析的 scope 初筛结果' };
      }
      if (!['trivial', 'local', 'cross-file', 'needs-clarification'].includes(body.scope)) {
        return { kind: 'unreliable', reason: `LLM 返回了无效的 scope 值: ${String(body.scope)}` };
      }
      return {
        kind: 'reliable',
  ...
  async preFilterNonFindingDiscussion(
    ...
    const payload: PreFilterNonFindingPromptPayload = { discussionBody, discussionNoteCount };
    try {
      const json = await this.llmClient.completeJson(
        ...
        {
          type: 'object',
          properties: { isProbablyNonFinding: { type: 'boolean' }, reason: { type: 'string' } },
          required: ['isProbablyNonFinding', 'reason'],
        }
      );
      const body = this.parseSimpleJson(json);
      if (!body || typeof body.isProbablyNonFinding !== 'boolean') {
        return { kind: 'unreliable', reason: 'LLM 返回了不可解析的非 finding 过滤结果' };
      }
      return {
        kind: 'reliable',
